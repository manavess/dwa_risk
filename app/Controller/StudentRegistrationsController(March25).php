...
// extract values in single quotes separated by comma
                    ...
                    $checkmarks = '';
                    /* Start for checking the fail students */
                    ...
                    /* End for checking the fail students */
...
// Student Prefered Colleges option
                                ...
                                if ($this->StudentPreferedColleges->saveAll($collegeData)) {
                                    $msg .= 'Added colleges succcessfully';
                                } else {
...
//end of getting values out of grade
                    ...
                    if (!empty($data)) {
                        ...
                        $msg .= "Please select subject to save data";
...
//print_r($year);
        ...
        if (empty($certificateId) || empty($secstdname)) {
            $this->Session->setFlash(__('Proper data not available with us. Please consult to Ministry'));
            $this->redirect('/StudentRegistrations/login');
        }
...
//$this->set('secstdname',$secstdname);
        ...
        $cities = $this->StudentRegistration->City->find('list');
        $states = $this->StudentRegistration->State->find('list');
        $countries = $this->StudentRegistration->Country->find('list', array('conditions' => array('Country.name' => 'South Sudan')));
        $groupSubjects = $this->StudentRegistration->GroupSubject->find('list');
        $employees = $this->StudentRegistration->Employee->find('list');
...
// $student['name']=@$studentsub[0]['Subject']['name'];
            ...
            $qualifying_subjects = $this->Subject->find('all', array('fields' => array('id', 'name'), 'conditions' => array('Subject.status' => 'Y')));
            $this->set('qualifying_subjects', $qualifying_subjects);
...
//}
    ...
    public function getcollegelist() {
        ...
        $subcount = count(explode(',', $totalsubjectId));
...
//$group_subject_data = $this->GroupSubSubject->query('select group_concat(subject_id) as subject_code, group_subject_id from group_sub_subjects group by group_subject_id having subject_code="' . $totalsubjectId . '" order by subject_id');
            ...
            if (!empty($group_subject_data)) {
                foreach ($group_subject_data as $value) {
                    /* Given below is code for showing only diploma colleges if only 5 subjects marks exists for a student */
                    if ($subcount == 5) {
                        $diplomagrpsub = $this->GroupSubject->find('first', array('conditions' => array('GroupSubject.id' => $value['group_sub_subjects']['group_subject_id'], 'GroupSubject.course_id' => 2)));

                        if (!empty($diplomagrpsub)) {
            ...
            if (!empty($group_subject_id)) {
                ...
                $group_subject = $this->CollegeGroupSubject->find('all', array('conditions' => array('CollegeGroupSubject.group_subjects_id in (' . $val . ')')));
                /* check if seats are available or not */

                foreach ($group_subject as $col) {
                    $collegeid[] = $col['CollegeGroupSubject']['college_id'];
                }

                $uniquecolid = array_unique($collegeid);
                reset($uniquecolid);
                sort($uniquecolid);

                $allcolleges = $this->Colleges->find('all', array('conditions' => array('NOT' => array('Colleges.id' => $uniquecolid))));

                $this->set('nocolleges', $allcolleges);
...
//echo $totalseats;die;
                        ...
                        $allocatedseats = $this->StudentAlotment->find('count', array('conditions' => array('college_group_subject_id' => $group_subjects_filter['CollegeGroupSubject']['id'])));
...
//pr($collegeGroupSubjectIdArr); die;
    ...
    public function Allocation() {
        ...
        $this->loadModel('TempAllocations');

        $allotforyear = $_REQUEST['year'];

        $admyear = '';

        if (!empty($allotforyear) && $allotforyear == '2014') {
            $admyear = '2014';
        } else if (!empty($allotforyear) && $allotforyear == '2013') {
            $admyear = '2013';
        } else {
            $admyear = $_REQUEST['year'];
        }
...
//$this->StudentAlotment->deleteAll(array('allocation_year' => date('Y')));
            $groupsubjects = $this->StudentPreferedColleges->find('all', array(
                'fields' => array('DISTINCT college_id'),
                'conditions' => array('StudentPreferedColleges.college_id <> 0'),
...
//for all college preferences which is maximum 15 the loop will check and run for student allotment.
                for ($a = 1; $a < 16; $a++) {
                    //$d=1;
                    foreach ($groupsubjects as $groupsubject) {

                        $group_subject_id = $groupsubject['StudentPreferedColleges']['college_id'];
                        // echo $group_subject_id;die;
                        $studentregistrationlist = $this->StudentPreferedColleges->find('all', array(
                            ...
                            'conditions' => array('NOT' => array('StudentPreferedColleges.college_id' => '0'),
                                'StudentRegistration.nationality' => 'South Sudan', 'year(StudentRegistration.date_of_certificate)' => $admyear,
                                'StudentPreferedColleges.college_id' => $group_subject_id, 'StudentPreferedColleges.college_preference' => $a)));
                        ...
                        if (!empty($studentregistrations)) {
                            ...
                            foreach ($studentregistrations as $studentregistration) {
                                ...
                                $alloted = $this->TempAllocations->find('first', array('conditions' => array('TempAllocations.student_registration_id' => $studentId)));
                                        ...
                                        foreach ($student['StudentPreferedColleges'] as $prefredcollege) {
                                            ...
                                            if (!empty($collegegroupsubid) && $collegegroupsubid != '0') {
                                                ...
                                                if (!empty($collegegroup)) {
                                                    ...
                                                    $allocatedseats = $this->TempAllocations->find('count', array('conditions' => array('TempAllocations.college_id' => $collegegroup['College']['id'], 'TempAllocations.allocation_year' => $admyear)));
                                                    $avaliableseat = $avaliableseat - $allocatedseats;
                                                    if ($avaliableseat > 0) {
                                                        $studentcount++;
                                                        $collegecount++;
                                                        ...
                                                        /*
                                                          ...
                                                          'course_id' => '7', //$groupsubject['CollegeGroupSubject']['course_id'],
                                                          'college_id' => $collegegroupsubid, //$collegegroup['CollegeGroupSubjects']['college_id'],
                                                          'college_group_subject_id' => 1, //$collegegroupsubid,
                                                          'grade' => $grade,
    ...
    function studenttempallocation($studentId) {
...
//                            }
        ...
        /* //Insert Into Database
          $this->TempAllocations->create(false);
          $this->TempAllocations->set(array(
          'id' => NULL,
          'student_registration_id' => $studentId,
          'college_id' => $collegegroupsubid,
          'total_marks' => $totalmarks,
          'compulsary_subject_marks' => $totalcompulsarymarks,
          'qualifying_subject_marks' => $totalqulifingmarks,
          'allocation_year' => $admyear
          ));
          $this->TempAllocations->save();
...
//* getting subjects list from student subjects table
    ...
    function changecollege($id = null, $stdalotmentID = null, $courseid = null) {
...
//$this->set('groupsubjects', $this->GroupSubject->find('list', array('order' => 'GroupSubject.name ASC')));
        ...
        /*         * ********Fetching all relevant colleges********* */
        ...
        if (!empty($group_subject_id)) {
            ...
            $group_subject = $this->CollegeGroupSubject->find('all', array('conditions' => array('CollegeGroupSubject.group_subjects_id in (' . $val . ')', 'CollegeGroupSubject.course_id' => $courseid)));
//pr($group_subject);die;
            foreach ($group_subject as $group_subjects_filter) {
                if (!empty($group_subjects_filter['CollegeGroupSubject']['id'])) {
                    $totalseats = $group_subjects_filter['CollegeGroupSubject']['no_of_seat'];
            ...
            $options = '';
            $options .= "<select name=data[StudentRegistration][college_id]>";
            foreach ($newgrpsubject as $data) {
                $options .= "<option value=" . $data['College']['id'] . "," . $data['CollegeGroupSubject']['id'] . ">" . $data['College']['college_code'] . '-' . $data['GroupSubjects']['name'] . "</option>";
            }
            $options .= "</select>";
        }
        $this->set('collegeoptions', $options);
        $this->set('colleges', $newgrpsubject);

        /*         * *******End of fetching colleges******** */
