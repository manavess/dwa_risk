...
class CollegesController extends AppController {
    ...
    public function beforeFilter() {
        parent::beforeFilter();
        $this->Auth->allow('generalcollegeview', 'course', 'saveGroupSubject', 'RemoveGroupSubject');
    ...
    public function index() {
        ...
        if (!empty($this->request->data) || !empty($this->passedArgs['university_name'])) {
            ...
            $this->Session->write('download_collegelist', $data);
                ...
                if ($this->College->save($this->request->data)) {
                    ...
                    if (!empty($collegesubgroupitem)) {
                        foreach ($collegesubgroupitem as $key => $val) {
                            $groupcoursesid = '';
                            if (!empty($key)) {
                                $groupcoursesid = $this->getcourse($key);
                            }
                            $collegegroupsubjectpair[] = array('college_id' => $Collegeid, 'group_subjects_id' => $key, 'course_id' => $groupcoursesid, 'no_of_seat' => $val, 'created_by' => $userid);
    ...
    public function download_list_of_colleges() {
        ...
        $collegelist = $this->Session->read('download_collegelist');
        unset($_SESSION['download_collegelist']);
        if (empty($collegelist)) {

            $this->College->recursive = 0;
            $collegelist = $this->College->find('all');
        }
        //
        $this->set('colleges', $collegelist);

        $download_date = date('d-m-Y');
        $this->layout = NULL;
        header("Content-Type:application/vnd.ms-excel");
        header("Content-Disposition: attachment;Filename=colleges_list" . $download_date . ".xlsx");
    }
    ...
    function saveGroupSubject() {
        ...
        if ($this->request->is('post') || $this->request->is('put')) {
            ...
            if (!empty($this->request->data['Colgrpsubid']) && !empty($iscollegegrpid)) {
                ...
                $groupcoursesid = '';
                if (!empty($this->request->data['group_sub_id'])) {
                    $groupcoursesid = $this->getcourse($this->request->data['group_sub_id']);
                }

                $this->request->data['CollegeGroupSubject']['course_id'] = $groupcoursesid;
    ...
    function getcourse($grpsubid) {
        $groupsubcourse = array();
        $courseid = '';

        $this->loadModel('GroupSubject');
        if (!empty($grpsubid)) {
            $groupsubcourse = $this->GroupSubject->find('first', array('fields' => array('course_id'), 'conditions' => array('GroupSubject.id' => $grpsubid)));

            $courseid = $groupsubcourse['GroupSubject']['course_id'];
        }
