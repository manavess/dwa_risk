...
/*from total compulsary subject 1 has been subtracted because from basic and specialized only one can be chosed at a time*/
        ...
        foreach ($subject as $key => $value) {
            ...
            //pr($subject);die;
            if(array_key_exists(0, $studentsubject)){
                
 		$option .= "<input type='hidden' class='subjects' id='subject{$i}' name='subjectid[]' value='{$studentsubject[$i]['subject_id']}',size='20px'><input type='text' name='percentage[]' value='{$studentsubject[$i]['marks']}' class='subject_marks' onblur='calculateValue()' maxlength='2' readonly='readonly'></span>";
                
            }else{
                $option .= "<input type='hidden' class='subjects' id='subject{$i}' name='subjectid[]' value='{$key}',size='20px'><input type='text' name='percentage[]' value='{$studentsubject[$i]['marks']}' class='subject_marks' onblur='calculateValue()' maxlength='2'></span>";
            }
            $i++;
        }
        $option .= "<select width='100px' class='subjects' name='subjectid[]' onchange='collegelist(),getmarks(this)'>";
      
        foreach($mathsarray as $key=>$value){
           
              if(!empty($key) && $studentsubject[$i]['subject_id']==$key){
        ...
        $option .= "</select>";
        if(!empty($markingtype) && array_key_exists(0, $studentsubject)){
            $option .= "<input type='text' name='percentage[]' value='{$studentsubject[$marksKey]['marks']}' class='subject_marks' onblur='calculateValue()' maxlength='2'>";
        }else{
              $option .= "<input type='text' name='percentage[]' value='{$studentsubject[$marksKey]['marks']}' class='subject_marks' onblur='calculateValue()' maxlength='2'>";
	...
	/*1 has been added to comply with the value of i in order to also count the maths subject array separted from subject loop*/	
        ...
        for ($i = 1; $i <= $optionalSubject; $i++) {
            ...
            $option1 .= "<span><select  class='subjects' id='subjectid{$i}' name='subjectid[]' style='margin:5px 5px 5px 5px;' onchange='collegelist(),getmarks(this)' ><option value=''>Select Subject</option>";

            foreach ($qualifying_subjects as $value) {
            	
            	if(!empty($studentsubject[$g]['subject_id']) && $studentsubject[$g]['subject_id']==$value['Subject']['id']) {
            	
                    $option1 .= "<option selected ='selected' value='{$value['Subject']['id']}'>{$value['Subject']['name']}</option>";
                    $subjvalue = $studentsubject[$g]['marks']; /*this is the selected subject marks and goes to input box next to option*/
                }else{
                    $subjvalue = $studentsubject[$g]['marks']; /*this is the selected subject marks and goes to input box next to option*/     
            ...
            $option1 .= "</select>";
            if(!empty($subjvalue)){
                $option1 .= "<input type='text' name='percentage[]'  class='subject_marks' value='{$subjvalue}' onblur='calculateValue()' maxlength='2' readonly='readonly'></span>";
            }else{
        ...
        foreach ($subject as $value) {
          if(!empty($markingtype) && $studentsubject[$a]['subject_id']==$value['Subject']['id']){
