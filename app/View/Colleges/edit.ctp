                ...
                foreach ($CollegeGroupSubject as $key => $val) {
                    ...
                    if ($val['CollegeGroupSubject']['group_subjects_id'] == $gsub['GroupSubjects']['id']) {
                        ...
                        <div class="<?php echo $class1; ?>">
                            ...
                            echo $this->Form->input('CollegeGroupSubject.id', array('name' => 'data[CollegeGroupSubject][id][]','id'=>'CollegeGroupSubjectId'.$i, 'value' => $val['CollegeGroupSubject']['id'], 'type' => 'hidden'));
                      
                            ?><label><?php echo $gsub['GroupSubjects']['name']; ?></label>
                                <?php
                            echo $this->Form->input('CollegeGroupSubject.no_of_seat', array('name' => 'data[CollegeGroupSubject][no_of_seat][]', 'type' => 'text', 'label' => false, 'div' => false, 'value' => $val['CollegeGroupSubject']['no_of_seat'], 'id'=>'grpsubvalue'.$i,'placeholder'=>'No of seats','onblur'=>'editgrpsubcheck('.$i.')'));
                            ?>
                        </div>
                    <?php
                        }else{?>
        ...
        if (!empty($excludedSubId)) {
            foreach ($excludedSubId as $key => $val) {
                if($b%2==0){
                    ...
                    echo $this->Form->input('CollegeGroupSubject.id', array('name' => 'data[CollegeGroupSubject][id][]','id'=>'CollegeGroupSubjectId'.$i, 'value' => '', 'type' => 'hidden'));
                    ?><label><?php echo $groupsubjects[$key]['GroupSubjects']['name'];?></label>
                    <?php

                    echo $this->Form->input('CollegeGroupSubject.no_of_seat', array('name' => 'data[CollegeGroupSubject][no_of_seat][]', 'type' => 'text', 'label' => false, 'div' => false, 'id'=>'grpsubvalue'.$i,'readonly','placeholder'=>'No of seats','value'=>'','onblur'=>'editgrpsubcheck('.$i.')'));
                    ?> </div>
                        <?php
                 }else{?>
                        <div class="<?php echo $class2; ?>">
...
if($("#grpsubid"+i).is(':checked')==true){
	...
	$("#grpsubvalue"+i).removeAttr('placeholder','No of seats');
        if($("#grpsubvalue"+i).val()!=''){
        $.ajax({
            type: 'POST',
            data: {collegeID: $("#CollegeId").val(),Colgrpsubid:$("#CollegeGroupSubjectId"+i).val(),group_sub_id:$("#grpsubid"+i).val(),no_of_seat:$("#grpsubvalue"+i).val()},
            url: "<?php echo $this->webroot; ?>colleges/saveGroupSubject",
            success: function(data) {
                data = JSON.parse(data);
                alert(data.msg);
               $("#CollegeGroupSubjectId"+i).val(data.Colgrpsubid);
            }
        });
        return true;
        }
	...
	$("#grpsubvalue"+i).attr('readonly','readonly');
        ...
        if($("#CollegeGroupSubjectId"+i).val()!=''){
        $.ajax({
            type: 'POST',
            data: {Colgrpsubid:$("#CollegeGroupSubjectId"+i).val()},
            url: "<?php echo $this->webroot; ?>colleges/RemoveGroupSubject",
            success: function(data) {
                data = JSON.parse(data);
                ...
                if(data.Colgrpsubid==''){
                    ...
                    $("#grpsubid"+i).attr('checked','checked');
                    alert(data.msg);
                }
