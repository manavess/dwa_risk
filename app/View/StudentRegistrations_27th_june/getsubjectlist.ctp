...
/*from total compulsary subject 1 has been subtracted because from basic and specialized only one can be chosed at a time*/
        ...
        foreach ($subject as $key => $value) {
 		...
 		$option .= "<input type='hidden' class='subjects' id='subject{$i}' name='subjectid[]' value='{$key}',size='20px'><input type='text' name='percentage[]' value='{$studentTotalsubmarks[$i]['marks']}' class='subject_marks' onblur='calculateValue()' maxlength='2'></span>";
            }else{
